...
- Un fichero que únicamente incluya los correos electrónicos.
  ...
  ```bash
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat 107983_aariclist_25k.csv | sed '
s/^"//
s/".*$//'

ó

dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat 107983_aariclist_25k.csv | sed -E 's/"(.*)",".*",".*"/\1/'
  ```

- Un fichero que incluya solo los correos electrónicos de los usuarios cuyo nombre no está en blanco.
  
  ```bash
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat 107983_aariclist_25k.csv | sed -E '
/""/d
s/^"//
s/".*//'
  ```
...
```
```bash
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ echo '{ "correos":[' > mails.json

dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat lista.csv | sed -E 's/^"(.*)","(.*)","(.*)"$/ {correo: "\1", nombre: "\2\3"},/' >> mails.json

dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ echo "]}" >> mails.json
```
...
```bash
1.
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat /usr/share/dict/spanish | grep -E '^(..)\1$'

2.
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat /usr/share/dict/spanish | grep -E '^(.)(.)(.)\3\2\1$'

3.
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat /usr/share/dict/spanish | grep -E '^(.)(.).\2\1$'

4.
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat /usr/share/dict/spanish | grep -Ec '[^aeiouáéíóúü]{3}'

5.
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat /usr/share/dict/spanish | grep -Ec '^.$'
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat /usr/share/dict/spanish | grep -Ec '^..$'
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat /usr/share/dict/spanish | grep -Ec '^...$'
...
```
...
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat /proc/meminfo | sed -E 's/^(.*):[[:blank:]]*(.*)$/"\1": "\2",/'
...
dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat el_quijote.txt | sed 's/ /\n/g; s/,//g; s/;//g; s/://g; s/\.//g' | sort | uniq | wc -l

o

dmiguelez@A201_PC09:~/Practicas_ISO_v2/PR06$ cat el_quijote.txt | sed 's/ /\n/g; s/[,;:.!]//g' | sort | uniq | wc -l
